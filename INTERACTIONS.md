...
| Tap a **tile not yet ripe**                          | its worth, the ripening rule, its colour's power, its rarity line             | → **says to hold it** (2026-09-25); the lens is a hold                                                                  |
| **Hold** a placed tile (~250 ms)                     | —                                                                             | → **lights its ground while the finger is down**; again lets go (2026-09-25, `HexField`'s timer, `shell/tap#holdMeans`) |
...
| FIT ⇄ HERE camera toggle                 | yes                        | ✓, and then **replaced** 2026-09-08. FIT and HERE were both views the button invented, so an arranged board could not be got back. Three stops now — DEFAULT, FLAT (2d of the player's own pan and zoom), MY VIEW (the board their hands last left). Marc: _"the third one is always my own custom view so that if we toggle with this button we never lose the camera"_                                                                |
...
| MENU over the board                      | ✗ (it had ♪ and ?)         | ✓ 2026-08-30 — TOP-RIGHT, out of the arc a thumb sweeps: one door onto a short list (SOUND, HOW TO PLAY, MORE) that opens as a drawer under the stat row. A door on the stack, so Escape and Android BACK close it **In the header since 2026-09-25** (Marc: _"place it with the header"_), at the stat row's end while a run is played; the walked ending, which has no header, keeps the corner                                       |
| THE GROUND YOU WALKED, on the ending     | a picture of the board     | ✓ 2026-08-30 — a DOOR onto the live board: the ending steps aside to a bar and the real board takes the screen back, pan and pinch and FIT. Marc: _"i dont want a picture i want to actual screengame where we can move around"_ **The ending's picture is that door since 2026-09-25**, at the top, and REPLAY is a button on the board it opens (Marc: _"agglomerate with replay with buttons inside replay. make it a top feature"_) |
...
| `0`                             | the VIEW button's own cycle — DEFAULT, FLAT, MY VIEW               |
