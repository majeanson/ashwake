# NEXT-HISTORY.md — the closed half of `NEXT.md`, kept for its citations

**Nothing in this file is a to-do.** Retired from `NEXT.md` on 2026-09-24
(`LOG.md` Session 115), when that file had reached 2351 lines and its first
open item was a heading that said a bug was "STILL THERE" nine days after it
was fixed — the stale open-list its own opening paragraph warns about.

It is kept whole rather than deleted because the code cites it: docblocks point
at `NEXT.md` §0, §1, §1a, §2, §3, §4, §5 and §5b–§5d well over a hundred times,
and the section numbers below are those sections, unedited. **A citation to
`NEXT.md §N` in the code or in `LOG.md` means THIS file, §N.** Every item still
open when it was retired was carried to `NEXT.md` and checked against the code
on the way; if something below looks open and is not there, it was ruled,
built, or found to be taste with no defect under it — and `NEXT.md` §6 names
the last kind.

The text as it stood on 2026-09-24 follows.
