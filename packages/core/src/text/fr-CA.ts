...
Rien de neuf dedans. Une trouvaille ne donne que ce que tu ne portes pas déjà, et seulement sur ton propre monde.`,
  ...
  ui: {
    ...
    which: {
      ...
      doorShared: 'Le plateau de quelqu’un d’autre, joué nu.',
      doorDaily: 'Un plateau, tout le monde, aujourd’hui. Joué nu.',
    },
    ...
    lensPanel: {
      ...
      standing: 'EN PLACE',
      worth: (n) => `valeur ${d1(n)}`,
      tiles: (n) => `${n} tuile${pl(n, '', 's')}`,
      power: (n) => `pouvoir +${d1(n)}`,
      ripe: (count, worth) =>
      ...
      rows: {
        share: 'Part du plateau',
        perTile: 'Valeur par tuile',
        pockets: 'Poches mûres',
    ...
    handInstall: `Ashwake sur ton écran d’accueil${D}: touche Partager dans Safari, puis Sur l’écran d’accueil.`,
    backUpNote:
      'Tes mondes vivent seulement sur ce téléphone. SAUVEGARDER, dans les RÉGLAGES, les met à l’abri.',
    inApp:
