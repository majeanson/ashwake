...
Rien de neuf dedans. Une trouvaille ne donne que ce que tu ne portes pas déjà, et seulement sur ton propre monde.`,
  ...
  ui: {
    ...
    ending: {
      ...
      run: (n) => `PARTIE ${n}`,
      try: (n) => `ESSAI ${n}`,
      streak: (days) => `${nb(days)} JOURS D’AFFILÉE. Un nouveau plateau demain.`,
      comeBack: 'Un nouveau plateau demain.',
      tryAgain: 'REJOUER',
    ...
    handInstall: `Garde Ashwake sur ton écran d’accueil${D}: touche Partager au bas de Safari, puis Sur l’écran d’accueil. Le jeu s’ouvre sans les barres du navigateur, et tes mondes restent où ils sont.`,
    backUpNote:
      'Tes mondes vivent seulement sur ce téléphone, dans ce navigateur. Il n’y a aucun compte et rien n’est envoyé ailleurs, donc SAUVEGARDER dans les RÉGLAGES est la seule façon de les garder si tu effaces tes données de navigation ou changes de téléphone.',
    inApp:
