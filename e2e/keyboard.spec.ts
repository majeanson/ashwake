...
test('goes quiet while a panel is open, exactly as the board does', async ({ page }) => {
  ...
  const before = await lean(page);
  // The board's one door is MENU, which opens MORE directly; HOW TO PLAY is a
  // row on it.
  ...
  // MORE it opened from. The board only takes keys back once nothing is over
  // it, and MORE counts: it is a door like any other.
